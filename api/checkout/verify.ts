import { getOrderById, markLayawayBalancePaid, markUnitsSold, verifyOrderPayment } from "../../server/db.js";
...
// own unguessable UUID, same trust model already used by order-status.ts and
// pay-balance-status.ts (this email only ever goes to the owner's own inbox, never the
// customer). ?balance=1 verifies a layaway balance claim instead of the down payment.
